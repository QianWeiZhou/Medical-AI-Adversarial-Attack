...
class ImageDataset(data.Dataset):  # 需要继承data.Dataset
    def __init__(self, transform=None, setType=0, rootFolder='../data/',
        ...
        self.istd = img_std
        self.pn = setType
        self.negative = ['%s/negb9List.txt' % self.rootFolder, ]
        self.positive = ['%s/cancerList.txt' % self.rootFolder, ]
        ...
        self.set_len = self.posi_len + self.nega_len  # @UndefinedVariable
    ...
    def getIndex(self, index):
        ...
        Target = 0
        if index >= self.posi_len:
            Target = 1
            index = self.nega_perm[(index - self.posi_len) % self.nega_len]
        else:
