...
checkfolder = './checkfolder-positiveLookingGenerator/'
...
# ReadModelFrom = './../Breast2020Check/checkfolderV1728posPal41/'
ReadModelFrom = None
...
def train(modelList,paramList,data_loader,epoch=0,modelState=None):
    ...
    for pX,nX,pT,nT in data_loader:
        ...
        nX, pX = Variable(nX), Variable(pX,requires_grad=True)
        ...
        if (iter_count+epoch)%modelState['GN_pass'] == 0 and ( _step_%10 >= 1 ):
            ...
            fX = vfOutCat(fX)
            Xnoise = fX[:,[0],:,:] - nX 
            XnoiseM = torch.mean(nX**2)  # @UndefinedVariable
